...
describe('Snake', () => {
    ...
    describe('move', () => {
        describe('from default direction to', () => {
            ...
            expect(makeReducerTest([null], [[1, 1]]))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
            
            test('default to Right', () =>
            expect(makeReducerTest([Directions.Right], [[1, 1]]))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
            
            test('default to Down', () =>
            expect(makeReducerTest([Directions.Down], [[1, 1]]))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
    
            test('default to Left', () =>
            expect(makeReducerTest([Directions.Left], [[1, 1]]))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
    
            test('default to Up', () =>
            expect(makeReducerTest([Directions.Up], [[1, 1]]))
        ...
        describe('from Right to', () => {
            ...
            test('Right', () =>
            expect(makeReducerTest([Directions.Right], [[1, 1]], { direction: Directions.Right }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
            
            test('Down', () =>
            expect(makeReducerTest([Directions.Down], [[1, 1]], { direction: Directions.Right }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
    
            test('Left', () =>
            expect(makeReducerTest([Directions.Left], [[1, 1]], { direction: Directions.Right }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
    
            test('Up', () =>
            expect(makeReducerTest([Directions.Up], [[1, 1]], { direction: Directions.Right }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [0, 1] })]}));
        });

        describe('from Down direction to', () => {
            test('default direction', () =>
            expect(makeReducerTest([null], [[1, 1]], { direction: Directions.Down }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
            
            test('Right', () =>
            expect(makeReducerTest([Directions.Right], [[1, 1]], { direction: Directions.Down }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
            
            test('Down', () =>
            expect(makeReducerTest([Directions.Down], [[1, 1]], { direction: Directions.Down }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
    
            test('Left', () =>
            expect(makeReducerTest([Directions.Left], [[1, 1]], { direction: Directions.Down }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 0] })]}));
    
            test('Up', () =>
            expect(makeReducerTest([Directions.Up], [[1, 1]], { direction: Directions.Down }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
        });

        describe('from Left direction to', () => {
            test('default direction', () =>
            expect(makeReducerTest([null], [[1, 1]], { direction: Directions.Left }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 0] })]}));
            
            test('Right', () =>
            expect(makeReducerTest([Directions.Right], [[1, 1]], { direction: Directions.Left }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 0] })]}));
            
            test('Down', () =>
            expect(makeReducerTest([Directions.Down], [[1, 1]], { direction: Directions.Left }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [2, 1] })]}));
    
            test('Left', () =>
            expect(makeReducerTest([Directions.Left], [[1, 1]], { direction: Directions.Left }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 0] })]}));
    
            test('Up', () =>
            expect(makeReducerTest([Directions.Up], [[1, 1]], { direction: Directions.Left }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [0, 1] })]}));
        });

        describe('from Up direction to', () => {
            test('default direction', () =>
            expect(makeReducerTest([null], [[1, 1]], { direction: Directions.Up }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [0, 1] })]}));
            
            test('Right', () =>
            expect(makeReducerTest([Directions.Right], [[1, 1]], { direction: Directions.Up }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 2] })]}));
            
            test('Down', () =>
            expect(makeReducerTest([Directions.Down], [[1, 1]], { direction: Directions.Up }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [0, 1] })]}));
    
            test('Left', () =>
            expect(makeReducerTest([Directions.Left], [[1, 1]], { direction: Directions.Up }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [1, 0] })]}));
    
            test('Up', () =>
            expect(makeReducerTest([Directions.Up], [[1, 1]], { direction: Directions.Up }))
                .toMatchObject({ cells: [expect.objectContaining({ coordinate: [0, 1] })]}));
        });
    });
